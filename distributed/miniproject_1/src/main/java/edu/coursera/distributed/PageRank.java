    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRanks = sites.
                ...
                .flatMapToPair(kv -> {
                    ...
                    LinkedList<Tuple2<Integer, Double>> contributions = new LinkedList<>();

                    Iterator<Integer> edgeIter = website.edgeIterator();
                    while (edgeIter.hasNext()) {
                        final int target = edgeIter.next();
                        contributions.add(new Tuple2<>(target,
                                currRank / website.getNEdges()));
                    }
